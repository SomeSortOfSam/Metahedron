extends Reference
class_name TurnManager

var evil_turn := false
var turns := 0 setget set_turns

signal turn_ended

func subscribe(map : Map):
	for person in map.people.values():
		_on_map_person_added(person)
	var _connection = map.connect("person_added",self,"_on_map_person_added")

func set_turns(new_turns):
	turns = new_turns
	if turns <= 0:
		emit_signal("turn_ended")

func _on_map_person_added(person : Person):
	turns += 1 if person.is_evil == evil_turn else 0
	var _connection = person.connect("has_set_end_turn", self,"_on_person_set_end_turn")
	_connection = person.connect("has_attacked", self, "_on_person_has_attacked")
	_connection = person.connect("new_turn", self, "_on_person_new_turn")
	_connection = connect("turn_ended", person, "reset_turn", [evil_turn])

func _on_person_set_end_turn(turn_ended : bool):
	self.turns -= 1 if turn_ended else -1

func _on_person_has_attacked():
	self.turns -= 1

func _on_person_new_turn():
	self.turns += 1
