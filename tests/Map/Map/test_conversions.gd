...
func run_conversion_test(map : Map, map_point : Vector2, params):
	...
	assert_eq(map.world_to_map(world_point),map_point,"World to Map convertion at " + str(map_point) + MapTestUtilites.get_parameter_description(params))
	assert_eq(map.map_to_world(map_point),world_point,"Map to World convertion at " + str(map_point) + MapTestUtilites.get_parameter_description(params))
